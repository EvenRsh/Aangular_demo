...
http.createServer(function(request, response) {
	...
	function insert() {
		console.log('INSERT INTO `class`(`skill`, `name`) VALUES ("' + params.skill + '","' + params.name + '")');
		//插入
	...
	function del() {
		connection.query('DELETE FROM `class` WHERE id = ' + params.id, function(error, results, fields) {
			if(error) throw error;
			console.log('The solution is: ', results);
			//connection.end();
			//查询
	...
	function selectDetail() {
		connection.query('SELECT * FROM class where id = ' + params.id, function(error, results, fields) {
			if(error) throw error;
			console.log('结果集: ', results);
			obj.lists = results
				//connection.end();
	...
	function edit() {
		connection.query('UPDATE `class` SET `skill`="' + params.skill + '",`name`= "' + params.name + '" WHERE id = ' + params.id, function(error, results, fields) {
			if(error) throw error;
			console.log('结果集: ', results);
			//obj.lists = results
			//connection.end();
